...
- **阈值有外部证据。** 去重阈值 0.6 在 1062 条带人工标注的 SQuAD 问句上测过：召回 1.000、误报 0.004，且在困难负样本上是最优值。测法和局限见 [`benchmarks/`](benchmarks/)。
...
- **相似度是字符 n-gram（Jaccard）**，不含相同字符的改写会被当成两个问题。对着为绕开字面匹配而专门构造的句子对，98% 会被判为"同一问题"——这是这条技术路线的硬上限。可用 `--similarity` 换成你自己的实现。
- **v0.5 换了度量，因为旧的会在长输入上静默失效。** v0.4 及之前用的是重叠系数：它除以 `min(|A|,|B|)`，而长文本本来就满是常见 bigram——实测误报率随长度从 **22%**（<40 字符）单调爬到 **75%**（>600 字符），在 9,236 条真实用户问句上**把 93% 的语料并成了一个簇**，而 `build` 会把它当成功报告。换成 Jaccard 后，同一批数据整体误报从 **0.3411 降到 0.0023**（148 倍），带标签的改写数据上 F1 也更高。**代价是短碎片不再匹配**（`退款政策` 不再并进 `你们的退款政策是什么？`），样本日志因此从 33 个簇变成 39 个——多出 6 个用例。这个取舍是刻意的：**漏合并只是多一个冗余用例，错合并会让一个用例静默代表两个无关问题**。完整推导（含一个试过又扔掉的方案）见 [`benchmarks/real_traffic.md`](benchmarks/real_traffic.md)。
- **单链接会把小的误报放大。** 在 1062 条**互不重复**的 SQuAD 问句上，0.6 的配对误报率只有 0.4%，但单链接把这 0.4% 放大成 **54% 的问句被并进某个簇**、最大簇 425 条。`report.md` 里会给出你这份日志的对应数字（最大簇、被并入比例）。**没有自动告警**——因为"短碎问法离代表很远"既是合法链条也是错误合并的特征，分不开；理由写在 `ClusterHealth` 的注释里。
...
- **信号层还没被外部数据验证过。** 上面那个评估只测了聚类和去重。信号（差评、重试、延迟、成本）决定哪些 trace 值得测，而公开数据里**没有一份同时具备原始文本和运维字段**——服务端 trace 有延迟但抹掉了文本，对话日志有文本但没有延迟。所以"什么值得测"这一半，目前仍然只有那 42 行样本日志作证据。
...
改了会影响产物的代码之后，跑 `python rebuild.py` —— 它会按固定顺序重建
`evalset/`、`examples/` 依赖的指标文件和 `benchmarks/results.md`，并检查有没有混进 BOM 或 CRLF。
（我漏跑过一次，CI 的字节一致性断言直接挂了，所以把它写成了脚本。）
