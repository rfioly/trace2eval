...
- **相似度是字符 n-gram**，不含相同字符的改写会被当成两个问题。对着为绕开字面匹配而专门构造的句子对，它有 98% 会被判为"同一问题"——这是这条技术路线的硬上限。可用 `--similarity` 换成你自己的实现。
- **⚠️ 长输入会让它彻底失效，而且是静默的。** 用的重叠系数除以 `min(|A|,|B|)`，而长文本本来就满是常见 bigram——所以**两个毫不相关的长输入也能到 0.6~0.75**。在 9,236 条真实用户问句上实测：误报率随长度从 **22%**（<40 字符）单调爬到 **75%**（>600 字符），默认阈值把 **93% 的问句并成了同一个簇**，而 `build` 会把它当成功报告给你。**换 Jaccard 能治好这个，但会毁掉短碎片匹配**（实测：样本日志合法簇保留率从 82% 掉到 36%）——**没有免费的解，目前没修**。细节和三个可选方案见 [`benchmarks/real_traffic.md`](benchmarks/real_traffic.md)。**如果你的日志有长输入（粘文档、RAG 上下文、代码），现在别信这个工具的聚类结果。**
