...
- **⚠️ Long inputs break it completely, and silently.** The overlap coefficient divides by `min(|A|, |B|)`, and long texts are mostly common bigrams — so **two entirely unrelated long inputs score 0.6–0.75**. Measured on 9,236 real user questions: the false-positive rate climbs monotonically with length from **22%** (under 40 characters) to **75%** (over 600), and at the default threshold **93% of the corpus collapses into a single cluster** — which `build` reports as a successful run. **Swapping in Jaccard fixes that and destroys short-fragment matching** (measured: legitimate cluster pairs kept drops from 82% to 36%). **There is no free fix and none has been applied.** Details and three candidate approaches: [`benchmarks/real_traffic.md`](benchmarks/real_traffic.md). **If your log contains long inputs — pasted documents, retrieval context, code — do not trust the clustering today.**
