...
- **Similarity is character n-grams (Jaccard)**, so paraphrases sharing no characters score zero. Against sentence pairs built specifically to defeat string matching, 98% are called "the same question" — that is the ceiling on this approach. Use `--similarity` to swap in your own matcher.
- **v0.5 changed the measure, because the old one failed silently on long inputs.** Up to v0.4 it was the overlap coefficient, which divides by `min(|A|, |B|)` — and long texts are piles of common bigrams. Measured: the false-positive rate climbs monotonically with length from **22%** (under 40 characters) to **75%** (over 600), and on 9,236 real user questions it fused **93% of the corpus into a single cluster** while `build` reported a successful run. Under Jaccard the same corpus's overall rate falls from **0.3411 to 0.0023** — 148x — and F1 on labelled paraphrase data improves too. **The cost is that short fragments no longer match** (`退款政策` no longer joins `你们的退款政策是什么？`), which takes the sample log from 33 clusters to 39. That trade is deliberate: **a missed merge costs one redundant case, while a false merge produces a case silently standing for two unrelated questions.** Full derivation, including one approach that was tried and dropped, is in [`benchmarks/real_traffic.md`](benchmarks/real_traffic.md).
